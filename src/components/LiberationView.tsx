...
export const LiberationView: React.FC<LiberationViewProps> = ({ initialEmotion, onStartAnimation }) => {
  ...
  return (
    ...
    >
      ...
      <div className="bg-white dark:bg-[#1F2E33] border border-[#2F3E46]/10 dark:border-white/10 rounded-2xl p-3.5 space-y-1.5 shadow-sm">
        <div className="flex items-center gap-2">
          {emotionInfo ? (
            ...
            <span className="text-[11px] font-semibold text-[#5E8B7E] bg-[#5E8B7E]/15 px-2.5 py-0.5 rounded-full flex items-center gap-1.5">
              <Sparkles className="w-3 h-3" />
              Guía de expresión
            </span>
          )}
          <span className="text-[11px] text-[#5A6D75] dark:text-white/40 font-medium">¿Qué lo provoca?</span>
        </div>

        <p className="text-xs text-[#2F3E46] dark:text-white/80 leading-relaxed">
          {emotionInfo
            ? emotionInfo.prompt
      ...
      <div className="bg-white dark:bg-[#1F2E33] border border-[#2F3E46]/10 dark:border-white/10 rounded-3xl p-4 sm:p-5 flex flex-col gap-3 shadow-sm">
        <div className="flex items-center justify-between pb-1 border-b border-[#2F3E46]/10 dark:border-white/10">
          <div className="flex items-center gap-1.5 text-xs text-[#5A6D75] dark:text-white/60">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-[#5E8B7E] hover:text-[#4D7A6D] font-medium transition-colors px-2.5 py-1 rounded-lg bg-[#5E8B7E]/10"
          >
        ...
        <textarea
          ...
          className="w-full bg-transparent border-none outline-none text-[#2F3E46] dark:text-white text-sm leading-relaxed resize-none placeholder:text-[#5A6D75]/50 dark:placeholder:text-white/30 focus:ring-0"
        />

        {/* Footer */}
        <div className="flex items-center justify-between pt-2 border-t border-[#2F3E46]/10 dark:border-white/10 text-[11px] text-[#5A6D75] dark:text-white/50">
          <span id="wordCount" className="font-mono text-[#5E8B7E] font-semibold">
            {wordCount} {wordCount === 1 ? 'palabra' : 'palabras'}
          </span>
          <span className="italic text-[#5A6D75] dark:text-white/50 text-right truncate max-w-[200px]">
      ...
      <div className="space-y-3">
        <p className="text-xs font-semibold text-[#2F3E46] dark:text-white/80 uppercase tracking-wider">
        ...
        <div className="grid grid-cols-3 gap-2.5">
          ...
          <button
            ...
            className={`p-3 rounded-2xl border flex flex-col items-center gap-2 text-center transition-all ${
              selectedPractice === 'fuego'
                ? 'bg-[#5E8B7E]/15 border-[#5E8B7E] text-[#2F3E46] dark:text-white ring-1 ring-[#5E8B7E]'
                : 'bg-white dark:bg-[#1F2E33] border-[#2F3E46]/10 dark:border-white/10 text-[#5A6D75] dark:text-white/60 hover:bg-[#5E8B7E]/5'
            }`}
          >
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${
              selectedPractice === 'fuego' ? 'bg-[#5E8B7E] text-white' : 'bg-[#5E8B7E]/10 text-[#5E8B7E]'
            }`}>
          ...
          >
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${
              selectedPractice === 'mar' ? 'bg-[#5E8B7E] text-white' : 'bg-[#5E8B7E]/10 text-[#5E8B7E]'
            }`}>
          ...
          >
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${
              selectedPractice === 'globo' ? 'bg-[#5E8B7E] text-white' : 'bg-[#5E8B7E]/10 text-[#5E8B7E]'
            }`}>
        ...
        <div className="bg-[#5E8B7E]/10 border border-[#5E8B7E]/20 rounded-2xl p-3 text-center">
          <p id="practiceDescription" className="text-xs text-[#5E8B7E] italic font-medium">
